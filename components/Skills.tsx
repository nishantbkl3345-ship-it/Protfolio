        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12">
          {skillsData.map((category) => (
            <div key={category.category}>
              <h3 className="text-pink-500 font-semibold text-sm uppercase tracking-wider mb-4">
                {category.category}
              </h3>
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill) => (
                  <span
                    key={skill}
                    className="bg-white border border-pink-100 text-gray-700 text-sm px-4 py-2 rounded-full hover:border-pink-500 hover:text-pink-500 transition-colors cursor-default"
                  >
                    {skill}
                  </span>
                ))}
